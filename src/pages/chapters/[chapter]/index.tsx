...
import { useVerses } from "@/services/fetchers";
import { StateOptions, StateResult } from "@/types/state";
import { Quiz, QuranChapter } from "@/types/content";
import { DataLoader } from "@/components/ui/data-loader";
import { DataError } from "@/components/ui/data-error";
import Result from "./_components/_result";
import Navbar from "./_components/_navbar";
import Choice from "./_components/_choice";
...
export async function getServerSideProps(context: NextPageContext) {
  ...
  const cookie: { session?: string } = parse(cookieStore);
...
export default function ChapterPage() {
  ...
  const chapter: QuranChapter | null = React.useMemo(() => {
    if (verses.isLoading || verses.isValidating || verses.error) {
      return null;
    }
